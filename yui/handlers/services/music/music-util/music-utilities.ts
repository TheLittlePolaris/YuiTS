...
export function timeConverter(duration: number): Promise<string | number> {
  return new Promise((resolve, _) => {
    ...
    if (totalMinutes < 60) {
      return resolve(`${totalMinutes >= 10 ? totalMinutes : `0${totalMinutes}`}:${seconds}`)
    } else {
      const hours = Math.floor(totalMinutes / 60)
      const minutesLeft = totalMinutes % 60
      return resolve(
        `${hours >= 10 ? hours : `0${hours}`}:${minutesLeft >= 10 ? minutesLeft : `0${minutesLeft}`}:${seconds}`
      )
    }
  })
}
...
export function printQueueList(queue: MusicQueue, start: number, end: number): Promise<string> {
  ...
  return new Promise<string>(async (resolve, _) => {
    ...
    for (let i = start; i <= end; i++) {
      ...
      result += `#${i}: **${song.title}** - \`(${await timeConverter(song.duration)})\`\n*Requested by \`${
        song.requester
      }\`*\n\n`
    }
    resolve(result)
