...
export class FeatureService {
  ...
  public async getPing(message: Message, pings: number[]): Promise<void> {
    ...
    const embed = await discordRichEmbedConstructor({
      title: "Status",
      description: `:revolving_hearts: **: \`${timeEnd -
  ...
  public async say(message: Message, args: Array<string>): Promise<void> {
    ...
    if (
      ...
      (isMyOwner(message.author.id) ||
        memberHasPermission(message.member, requiredPermissions))
    ) {
      ...
      if (yuiHasPermission(yui, requiredPermissions)) {
        const deleted = await message.delete();
        deleted.author.send("sorry but i will only speak for my Master.");
        return Promise.resolve();
      }
  ...
  public async tenorGif(message: Message, args: Array<string>) {
    ...
    const _arguments = args.join(" ");
    const result = await tenorRequestService(_arguments).catch(
      this.handleError
    );
