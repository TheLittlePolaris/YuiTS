...
export class MusicQueueService {
  ...
  async enqueueSongFromQuery(
    ...
    if (type === 'youtube') {
      const videoId = await this.youtubeInfoService.getYoutubeVideoId(inputArguments);
      if (!videoId) {
        stream.sendMessage(
      ...
      const song = (await this.soundcloudService.getSoundcloudInfoFromUrl(
        inputArguments,
        false
      )) as IYoutubeVideo;
      if (!song) {
  ...
  async loadAutoplayQueue(stream: MusicStream) {
    ...
    const relatedVideosMetadata = await this.youtubeInfoService.getVideoMetadata(
      ...items.map((index) => index.id.videoId)
    );
  ...
  async getYoutubePlaylist(message: Message, inputArguments: string) {
    const youtubePlaylistId = this.youtubeInfoService.getYoutubePlaylistId(inputArguments);
    ...
    if (youtubePlaylistId)
      ...
      const videoId = await this.youtubeInfoService.getYoutubeVideoId(inputArguments);
      if (videoId) playListVideos = await this.youtubeInfoService.getVideoMetadata(videoId);
    }
